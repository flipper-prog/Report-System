...
python3 -m report_system verify     # 장부 전수 감사 — 봉인 해시 재계산 + 불변 트리거 실동작 확인
...
| L5 소득·구매력 | `income` (KOSIS 또는 파일) | `KOSIS_API_KEY` (API 경로만) | `kosis_income` 또는 `income_file` |
...
입력(현장·거래·청약·공급·계획·소득·현장반응)
  ...
  → transactions 거래 정제(취소·중복·이상·특수) — 이상 판정은 **동시기(±6개월)**
                 같은 단지·면적대 기준        [정제 내역 리포트 표기]
  ...
  ├→ funnel      퍼널 병목 진단(기준선 대조 → 운영 문제 / 조건 문제 분류)
  → verdicts     4개 독립 판정(가격·수요·공급/환금성·촉매) × 4속성 — 단일 점수 합산 없음
  → robustness   판정 강건성: 가정(조정계수·LTV·실현률·실현성) 교란 후 동일 판정
                 함수 재실행 → 방향이 뒤집히는 판정을 '가정 의존'으로 표기
...
| 근거의 양을 가중치로 부풀리지 않는다 | 분양권 우선 가중(`pricing.py`)은 **분포 산출에만** 반영. 리포트는 '실제 거래'와 '유효표본'을 별도 열로 제시하고, 신뢰도 판정·최소 표본 게이트는 실제 거래 건수로만 한다 | P1-1·P1-5 |
...
| 병목은 기준선이 있을 때만 지목한다 | 퍼널 진단(`funnel.py`) — 기준선 미설정 시 판정 유보. 방문→계약 병목은 광고가 아닌 **조건 문제**로 분류해 시행사 안건으로 승격 | 3.5·10.5 |
| 상담원에게 근거 없는 문장을 주지 않는다 | 판매 논리 산출물(`salespack.py`) — 6축 중 근거가 산출된 축만 답변 생성, 나머지는 '확인 후 회신' | 제6장 |
| 결론은 가정 위에 서 있음을 밝힌다 | 강건성 검사(`robustness.py`) — 가정을 흔들어 **운영과 동일한 판정 함수**를 재실행. 방향이 뒤집히는 판정은 리포트 요약과 상담 문장 양쪽에 '가정 의존'으로 표시되고, 해당 축은 단정형 문장을 쓰지 않는다 | 5.8 판정 해석 |
...
| 소득·구매력 (L5) | **실측 중위소득 연동** — 분포 중심을 시군구 실측값으로 고정. 산포(σ)와 분포 형태(로그정규)는 여전히 가정이며 리포트에 병기 |
| 조정계수(연식·층·시점) | **교정 엔진 구현** — `calibrate` 가 헤도닉 회귀로 추정하고 홀드아웃 검증을 통과할 때만 교체. 미교정 상태는 모델 카드에 '예시값'으로 표기 |
| 단계 실현률·DSR 가정 | 파라미터 노출 + **강건성 검사 대상**. 실적 누적 전까지 예시값이므로, 이 가정을 흔들었을 때 판정이 뒤집히는지를 리포트 1-2에 매 회차 기록한다 |
...
| 정제 룰 버전화 | **구현 완료** (P2-3, 현행 `clean-1.1`). 이상·특수 판정은 동시기(±6개월) 기준 — 필터 효과가 시장 방향과 상관되지 않음 |
